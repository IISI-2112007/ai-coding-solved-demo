...
repo 已改為 public，GitHub Pages 已啟用：

```text
https://iisi-2112007.github.io/ai-coding-solved-demo/
```

PR #4 的 preview URL 已實測可開：
...
如果未來 repo 改回 private，請重新檢查 GitHub Pages 方案或改接外部 preview provider。
...
- PR comment 內有真正可點開的 preview URL。
- deployment status 有可用的 `environment_url`。

Provider 決策與備援路線請見：[PR Preview Provider 決策紀錄](preview-provider-decision.md)。
